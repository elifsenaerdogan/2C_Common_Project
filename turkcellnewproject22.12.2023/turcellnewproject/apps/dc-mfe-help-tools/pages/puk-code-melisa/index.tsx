...
const PukCodePagesPrivate = () => {
  ...
  const ApiWrite = () => {
    return axios.get('/api/content-service/v1/pagemanager/dc-help-service-puk-query-pagemanager');
  }

  const {isLoading: isPageDataLoading } = useQuery(
      'pageData', ApiWrite
  );

  useEffect(() => {
    const fetchData = async () => {
      // Eğer pageData henüz yüklenmemişse, burada yükleme işlemini gerçekleştir.
      if (!isPageDataLoading) {
        try {
          // setPageData(response.data);
          // Yukarıdaki satırı kapatıp, eğer response.data'yı kullanmanız gerekiyorsa bu satırı kullanabilirsiniz.
  ...
  const handleFormSubmitTC = <FormInputs> => {
    ...
    const {
        ...
        resolver: yupResolver(isChecked1 ? validationSchemaWithSimCard : validationSchemaWithTCNo),
    });
