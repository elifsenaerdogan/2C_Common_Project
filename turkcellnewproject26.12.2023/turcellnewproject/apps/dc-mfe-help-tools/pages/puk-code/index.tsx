...
const PukKoduSorgulama = () => {
  ...
  let validationSchemaWithtckn = Yup.object().shape({
  });

  let validationSchemaWithSimCard = Yup.object().shape({
  ...
  if (pageData) {
    validationSchemaWithtckn = Yup.object().shape({
      gsmno: Yup.string().required(pageData.labelDTOList["helptools.pukcode.erroremptymessage.input.phonenumber"]),
      tckn: Yup.string().required(pageData.labelDTOList["helptools.pukcode.erroremptymessage.input.tckimlikno"]),
  ...
  const handleBlurTc = () => {
    if (pageData) {
      if (tckn.trim() === '') {
        setFormErrors({
          ...formErrors,
          tckn: pageData.labelDTOList["helptools.pukcode.erroremptymessage.input.tckimlikno"]
        });

      } else if (tckn.length < 11) {
        setFormErrors({...formErrors, tckn: pageData.labelDTOList["helptools.pukcode.failmessage.input.tckimlikno"]});
      } else {
        setFormErrors({...formErrors, tckn: ''});
      }
      setIsFocused(false);
    }
  };
  ...
  const TCLabel = classNames(
    `text-legal-regular-medium`,
    styles.trkclAppPagePukContentWithtcknLabel
  )
  ...
  const handleFormSubmitTC = async (inputValue) => {
    ...
    const selectedValidationSchema = validationSchemaWithtckn
      ...
      <div className={styles.trkclAppPagePuk}>
          ...
          <div className={styles.trkclAppPagePukContent}>
            ...
            <div>
              ...
              {isChecked1 ? (
                ...
                isChecked2 ? (
                  <div className={styles.trkclAppPagePukContentWithtckn}>
                    <form onSubmit={handleSubmit(handleFormSubmitTC)}>
                      <div className={styles.trkclAppPagePukContentWithtcknFirstRow}>
                        <div className={styles.trkclAppPagePukContentInput}>
                          <AtomInput
                            label={pageData.labelDTOList["helptools.pukcode.withsim.input.phonenumber"]}
                            name="gsmno"
                        ...
                        <div className={styles.trkclAppPagePukContentInput}>
                          <AtomInput
                            ...
                            name="tckn"
                            value={tckn}
                            message={formErrors.tckn || validationErrorsTC.tckn}
                            onBlur={handleBlurTc}
                            onFocus={handleFocus}
                            onChange={(event) => handleInputChangeTC(event, setTckn)}
                            statusType={(formErrors.tckn || validationErrorsTC.tckn) ? 'error' : undefined}/>
                          <p
                            className={TCLabel}>{pageData.labelDTOList["helptools.pukcode.withidnumber.label.idnumber"]}</p>
                        </div>
                      </div>
                      <div className={styles.trkclAppPagePukContentWithtcknSecondRow}>
                        <div className={styles.trkclAppPagePukContentInput}>
                      ...
                      <div className={styles.trkclAppPagePukContentWithtcknThirdRow}>
                        <Captcha inputLabel="Güvenlik Kodu"
                                 isLoading={isLoading}
                                 refreshCaptcha={() => {
                                   console.log('Captcha yenilendi');
        ...
        {isVisiblePageResponse && pageData && (
          <div>
            ...
            <div className={styles.trkclAppPagePukContentWithtcknFourthRow}>
              <div></div>
              <AtomButton text={pageData.labelDTOList["helptools.pukcode.response.button"]} variant="secondary"
                          className={styles.trkclAppPagePukContentWithtcknButton}
              />
            </div>
